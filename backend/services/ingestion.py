...
class IngestionService:
    ...
    def ingest_all_files_from_github(self, owner: str, repo: str) -> Dict[str, Any]:
        """
        Ingest ALL files from a GitHub repository in a memory-efficient way.
        This includes all code files, configuration files, documentation, and more.
        Processes files and embeddings in small batches to avoid RAM spikes.
        Skips files that have already been processed (same SHA hash).
        ...
        Returns:
            Summary statistics of the ingestion process
        """
        logger.info(f"Starting FULL ingestion from GitHub: {owner}/{repo}")
        logger.info(f"Initial memory usage: {get_memory_usage()}")
        logger.info("=" * 60)
        logger.info("📋 INGESTION MODE: ALL FILES")
        logger.info("✅ Will process: ALL text-based files (code, config, docs, etc.)")
        logger.info("⏭️  Excluding: Binary files (images, archives, executables)")
        logger.info("=" * 60)
        repository_id = f"{owner}/{repo}"

        # Step 1: Find ALL files using ultra-fast method
        logger.info("Step 1: Finding all files in GitHub repository...")
        logger.info("🚀 Attempting ULTRA-FAST tree API search for ALL files...")

        try:
            all_files = self.github_service.find_all_files_fast(owner, repo, include_binary=False)
        except Exception as e:
            logger.error(f"Failed to retrieve files from repository: {e}")
            return {
                "status": "error",
                "message": str(e),
                "files_fetched": 0,
        ...
        for file_idx, file_info in enumerate(all_files, 1):
            ...
            try:
                ...
                file_size = file_info.get('size', 0)
                file_type = file_info.get('file_type', 'unknown')
                max_file_size = 1000000  # 1MB limit per file
                ...
                # **EARLY SHA CHECK** - Check if already processed BEFORE fetching content
                if file_sha and self.vector_client.file_already_processed(repository_id, file_path, file_sha):
                    logger.info(f"⏭️  Skipping {file_info['name']} - already processed (SHA: {file_sha[:8]})")
                    files_skipped += 1
                    continue
                ...
                try:
                    ...
                    logger.warning(f"Failed to fetch content for {file_path}: {fetch_error}")
                    files_skipped += 1
                    continue
                ...
                # Add character limit check for very large files
                max_file_chars = 1000000  # Maximum 1M characters
                if len(content) > max_file_chars:
                    logger.warning(f"⚠️  File content too large ({len(content)} chars, max: {max_file_chars}) - Skipping: {file_path}")
                    files_skipped += 1
                    del content
                ...
                # For markdown files, remove image references
                original_length = len(content)
                if file_type == 'documentation' and file_path.lower().endswith('.md'):
                    content = remove_images_from_markdown(content)
                    cleaned_length = len(content)
                    if original_length != cleaned_length:
                        logger.info(f"Removed images from {file_path}: {original_length} -> {cleaned_length} chars")
                ...
                try:
                    ...
                    # **TIMEOUT PROTECTION** using threading
                    from concurrent.futures import ThreadPoolExecutor, TimeoutError as FutureTimeoutError
                    ...
                    try:
                        with ThreadPoolExecutor(max_workers=1) as executor:
                            future = executor.submit(chunk_with_timeout)
                            chunks = future.result(timeout=10)  # 10 seconds timeout for larger files
                    except (FutureTimeoutError, TimeoutError):
                        logger.error(f"⏱️  TIMEOUT: Chunking took too long for {file_info['name']} - SKIPPING")
                        files_dropped_due_to_memory += 1
                        files_skipped += 1
                ...
                if not embedding_batch_failed:
                    files_processed += 1
                    processed_by_type[file_type] = processed_by_type.get(file_type, 0) + 1
                    logger.info(f"✓ Completed {file_info['name']} ({file_idx}/{len(all_files)})")
                else:
                    logger.warning(f"⚠️  Partially processed or skipped {file_info['name']} due to memory constraints")
                    files_skipped += 1
        ...
        logger.info(f"  Total processed: {files_processed}/{len(all_files)} files")
        for ft, count in sorted(processed_by_type.items()):
            logger.info(f"    📁 {ft}: {count}")
        logger.info(f"  Skipped: {files_skipped} files")
        if files_dropped_due_to_memory > 0:
            logger.info(f"  Dropped due to memory: {files_dropped_due_to_memory}")
    ...
    def ingest_org_repositories_all_files(self, org: str, keyword: str = "", max_repos: int = None) -> Dict[str, Any]:
        """
        Ingest ALL files from multiple repositories in an organization,
        optionally filtered by a keyword.

        This method retrieves ALL text-based files including:
        - Source code files (.py, .js, .ts, .java, .go, .rs, .bal, etc.)
        - Configuration files (.yaml, .yml, .json, .toml, .ini, etc.)
        - Documentation files (.md, .rst, .txt, etc.)
        - API definition files (.graphql, .proto, etc.)
        ...
        """
        logger.info("=" * 80)
        logger.info(f"Starting FULL bulk organization ingestion: {org} (keyword: '{keyword}')")
        logger.info("📋 INGESTION MODE: ALL FILES")
        logger.info("✅ Will process: ALL text-based files (code, config, docs, etc.)")
        logger.info("⏭️  Excluding: Binary files (images, archives, executables)")
        logger.info("=" * 80)
        ...
        # Step 2: Process each repository
        logger.info(f"Step 2: Processing {len(repositories)} repositories (ALL files)...")
        ...
        for i, repo_info in enumerate(repositories, 1):
            ...
            try:
                # Wait for memory to be available before processing next repo
                wait_for_memory(
                    threshold_percent=96.0,
                    check_interval=2.0,
                    timeout=120.0,
                    raise_on_timeout=True
                )

                # Ingest ALL files from this repository
                result = self.ingest_all_files_from_github(owner, repo_name)
                ...
                # Aggregate processed_by_type counts
                for ft, count in result.get("processed_by_type", {}).items():
                    all_processed_by_type[ft] = all_processed_by_type.get(ft, 0) + count
                ...
                results.append({
                    ...
                    "files_dropped_memory": result.get("files_dropped_memory", 0),
                    "embeddings_stored": result.get("embeddings_stored", 0),
                    "processed_by_type": result.get("processed_by_type", {})
                })
        ...
        logger.info("=" * 80)
        logger.info("FULL BULK INGESTION COMPLETED")
        logger.info("=" * 80)
        logger.info(f"Organization: {org}")
        logger.info(f"Keyword filter: '{keyword}'")
        logger.info(f"Repositories processed: {repos_processed}/{len(repositories)}")
        ...
        for ft, count in sorted(all_processed_by_type.items()):
            logger.info(f"    📁 {ft}: {count}")
        logger.info(f"Total files skipped: {total_files_skipped}")
        if total_files_dropped_memory > 0:
            logger.info(f"Total files dropped due to memory: {total_files_dropped_memory}")
        ...
        return {
            ...
            "total_files_dropped_memory": total_files_dropped_memory,
            "total_embeddings_stored": total_embeddings_stored,
            "processed_by_type": all_processed_by_type,
            "details": results
        }
