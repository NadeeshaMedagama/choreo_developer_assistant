# Documentation Index

Welcome to the DevChoreo documentation! All documentation has been organized into topic-based directories for easy navigation.

## 🗂️ Directory Structure

### **[01-getting-started/](./01-getting-started/)**
Everything you need to get DevChoreo up and running.
- Setup guides and prerequisites
- Quick start guides for all features
- Environment configuration
- Testing guides

**Start here if you're new!**

### **[02-features/](./02-features/)**
Detailed documentation for all DevChoreo features.
- Conversation Memory System
- Progressive Streaming Responses
- Source Citations & Filtering
- Monitoring & Observability
- Visual guides and diagrams

**Explore features in depth.**

### **[03-deployment/](03-deployment/)**
Guides for deploying DevChoreo in various environments.
- Choreo Platform deployment
- Docker deployment
- Frontend-backend connection
- Production readiness checklist

**Deploy with confidence.**

### **[04-troubleshooting/](./04-troubleshooting/)**
Solutions to common problems and known issues.
- Azure OpenAI error handling (429, 422, etc.)
- Memory and performance issues
- Source citation fixes
- Content filtering fixes

**Fix issues quickly.**

### **[05-implementation-notes/](./05-implementation-notes/)**
Technical implementation details and notes.
- Component documentation
- Configuration guides
- Performance optimizations
- Feature implementation status

**For developers and contributors.**

### **[06-migration-history/](./06-migration-history/)**
Historical documentation and security audits.
- Code migrations
- Project reorganizations
- Security audit reports
- Best practices learned

**Understand the evolution.**

### **[07-url-validation/](./07-url-validation/)**
URL validation feature documentation.
- Automatic URL validation in responses
- Trusted domains whitelist
- Private repository URL handling
- Architecture and implementation details

**Ensure working URLs in responses.**

### **[08-ai-models/](./08-ai-models/)**
Documentation about AI models used in the project.
- Azure OpenAI GPT-4 configuration
- Embedding models (text-embedding-ada-002, Sentence Transformers)
- Model switching and optimization
- Cost considerations

**Understand and configure AI models.**

### **[09-bug-fixes/](./09-bug-fixes/)**
Documented bug fixes and solutions.
- Issue descriptions and error messages
- Root cause analysis
- Fixes applied
- Prevention tips

**Learn from resolved issues.**

### **[10-changelog/](./10-changelog/)**
Version history and feature additions.
- URL validation feature (v1.5.0)
- Trusted domains whitelist (v1.5.1)
- Feature changelogs
- Migration notes

**Track changes and updates.**

## 🚀 Quick Navigation

### I want to...

**Get started quickly**
→ [01-getting-started/MAIN_README.md](./01-getting-started/MAIN_README.md)

**Set up the project**
→ [01-getting-started/SETUP_GUIDE.md](./01-getting-started/SETUP_GUIDE.md)

**Run the application**
→ [01-getting-started/RUN_PROJECT.md](./01-getting-started/RUN_PROJECT.md)

**Learn about conversation memory**
→ [02-features/CONVERSATION_MEMORY_IMPLEMENTATION.md](./02-features/CONVERSATION_MEMORY_IMPLEMENTATION.md)

**Understand streaming**
→ [02-features/STREAMING_IMPLEMENTATION.md](./02-features/STREAMING_IMPLEMENTATION.md)

**Deploy to production**
→ [03-deployment/CHOREO_DEPLOYMENT.md](03-deployment/CHOREO_DEPLOYMENT.md)

**Fix 429 errors**
→ [04-troubleshooting/TROUBLESHOOTING_429_ERRORS.md](./04-troubleshooting/TROUBLESHOOTING_429_ERRORS.md)

**See source code organization**
→ [05-implementation-notes/IMPLEMENTATION_SUMMARY.md](./05-implementation-notes/IMPLEMENTATION_SUMMARY.md)

**Validate URLs in responses**
→ [07-url-validation/URL_VALIDATION.md](./07-url-validation/URL_VALIDATION.md)

**Learn about AI models**
→ [08-ai-models/MODELS_DOCUMENTATION.md](./08-ai-models/MODELS_DOCUMENTATION.md)

**Fix private repo URLs**
→ [07-url-validation/FIX_PRIVATE_REPO_URLS.md](./07-url-validation/FIX_PRIVATE_REPO_URLS.md)

**View recent changes**
→ [10-changelog/](./10-changelog/)

## 📋 Complete Document List by Category

### Getting Started (9 docs)
- MAIN_README.md
- SETUP_GUIDE.md
- RUN_PROJECT.md
- CHOREO_QUICK_START.md
- ENV_FILE_LOCATION.md
- GOOGLE_CREDENTIALS_SETUP.md
- QUICK_START_CONVERSATION_MEMORY.md
- QUICK_START_CONVERSATION_HISTORY.md
- QUICK_START_INGESTION.md
- TESTING_SOURCES.md

### Features (14 docs)
- CONVERSATION_MEMORY_IMPLEMENTATION.md
- CONVERSATION_HISTORY_WITH_RETRIEVAL.md
- CONVERSATION_FLOW_DIAGRAM.md
- STREAMING_IMPLEMENTATION.md
- STREAMING_RESPONSES.md
- SOURCES_FEATURE.md
- INTELLIGENT_SOURCE_FILTERING.md
- INCREMENTAL_INGESTION.md
- MONITORING.md
- EDIT_COPY_IMPLEMENTATION.md
- EDIT_AND_COPY_QUESTIONS.md
- VISUAL_GUIDE.md
- SOURCES_VISUAL_GUIDE.md
- SOURCE_QUALITY_VISUAL_GUIDE.md

### Deployment (7 docs)
- CHOREO_DEPLOYMENT.md
- CHOREO_RUN_COMMAND.md
- DOCKER_README.md
- DOCKER_QUICK_REFERENCE.md
- FRONTEND_DIST_DEPLOYMENT.md
- FRONTEND_BACKEND_CONNECTION.md
- READY_TO_PUSH.md

### Troubleshooting (10 docs)
- TROUBLESHOOTING_429_ERRORS.md
- QUICK_FIX_429.md
- FIX_422_ERROR.md
- FIX_OPENCHOREO_FILTERING.md
- FIX_SOURCES_NOT_SHOWING.md
- CRASH_ANALYSIS_AND_FIXES.md
- MEMORY_FIX_SUMMARY.md
- MEMORY_LAG_FIX.md
- OVERFLOW_FIXES.md
- CHANGES_OPENCHOREO_FIX.md

### Implementation Notes (21 docs)
- IMPLEMENTATION_SUMMARY.md
- IMPLEMENTATION_COMPLETE.md
- IMPLEMENTATION_COMPLETE_02.md
- IMPLEMENTATION_CHECKLIST.md
- SETUP_COMPLETE.md
- INGESTION_README.md
- INGEST_WSO2_CHOREO_REPOS.md
- API_FILES_INGESTION_FEATURE.md
- ALL_MD_AND_API_FILES_INGESTION.md
- FRONTEND_README.md
- README_SOURCES.md
- QUICK_REFERENCE_SOURCES.md
- OPENAPI_ADDED.md
- COMPONENT_YAML_RESTRUCTURED.md
- SYSTEM_PROMPTS_UPDATED.md
- CHUNKING_LIMITS.md
- MARKDOWN_TABLES_SETUP.md
- MANUAL_SKIP_FEATURE.md
- AGGRESSIVE_SKIP_SUMMARY.md
- MEMORY_AWARE_FILE_DROPPING.md
- SPEED_OPTIMIZATIONS.md

### Migration History (4 docs)
- SCRIPTS_MIGRATION_SUMMARY.md
- TEST_MIGRATION_SUMMARY.md
- SECURITY_AUDIT_REPORT.md
- COMPLETE_REORGANIZATION.md

### URL Validation (5 docs)
- URL_VALIDATION.md
- URL_VALIDATION_ARCHITECTURE.md
- URL_VALIDATION_IMPLEMENTATION.md
- QUICK_START_URL_VALIDATION.md
- FIX_PRIVATE_REPO_URLS.md

### AI Models (1 doc)
- MODELS_DOCUMENTATION.md

### Bug Fixes (1 doc)
- BUGFIX_ASYNCIO_IMPORT.md

### Changelog (2 docs)
- CHANGELOG_URL_VALIDATION.md
- CHANGELOG_TRUSTED_DOMAINS.md
- TEST_MIGRATION_SUMMARY.md
- COMPLETE_REORGANIZATION.md
- SECURITY_AUDIT_REPORT.md

### Root Directory (2 docs)
- INDEX.md (this file)
- README_UPDATES_V2.md

## 🎯 Recommended Reading Paths

### For New Users
1. [Getting Started/MAIN_README.md](./01-getting-started/MAIN_README.md)
2. [Getting Started/SETUP_GUIDE.md](./01-getting-started/SETUP_GUIDE.md)
3. [Getting Started/RUN_PROJECT.md](./01-getting-started/RUN_PROJECT.md)
4. [Features/VISUAL_GUIDE.md](./02-features/VISUAL_GUIDE.md)

### For Developers
1. [Implementation Notes/IMPLEMENTATION_SUMMARY.md](./05-implementation-notes/IMPLEMENTATION_SUMMARY.md)
2. [Features/CONVERSATION_MEMORY_IMPLEMENTATION.md](./02-features/CONVERSATION_MEMORY_IMPLEMENTATION.md)
3. [Features/STREAMING_IMPLEMENTATION.md](./02-features/STREAMING_IMPLEMENTATION.md)
4. [Implementation Notes/FRONTEND_README.md](./05-implementation-notes/FRONTEND_README.md)

### For DevOps/Operators
1. [Deployment/CHOREO_DEPLOYMENT.md](03-deployment/CHOREO_DEPLOYMENT.md)
2. [Deployment/DOCKER_README.md](03-deployment/DOCKER_README.md)
3. [Features/MONITORING.md](./02-features/MONITORING.md)
4. [Troubleshooting/](./04-troubleshooting/)

## 🔄 Recent Updates

**December 2, 2025** - Documentation Reorganization
- ✅ Organized all docs into 6 topic-based directories
- ✅ Created README.md in each directory
- ✅ Updated INDEX.md with new structure
- ✅ Added quick navigation and reading paths
- ✅ Improved discoverability

**See:** [README_UPDATES_V2.md](./README_UPDATES_V2.md) for v2.0.0 changes

## 📝 Contributing Documentation

When adding new documentation:
1. Determine the appropriate category directory
2. Create the markdown file in that directory
3. Add a link to the directory's README.md
4. Update this INDEX.md if it's a major document
5. Follow the existing naming conventions

### Naming Conventions
- Use `SCREAMING_SNAKE_CASE.md` for compatibility
- Be descriptive: `FEATURE_NAME_IMPLEMENTATION.md`
- Use prefixes: `QUICK_START_`, `FIX_`, `TROUBLESHOOTING_`

## 🔗 External References

- **Main Project README**: [../../README.md](../../README.md)
- **Backend Services**: [../../backend/services/](../../backend/services/)
- **Frontend Components**: [../../frontend/src/](../../frontend/src/)
- **Monitoring Docs**: [../../backend/monitoring/](../../backend/monitoring/)

---

**Last Updated**: December 2, 2025  
**Total Documents**: 67 organized files  
**Organization**: Topic-based directories for easy navigation

